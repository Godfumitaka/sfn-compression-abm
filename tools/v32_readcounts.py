...
使い方  python3.12 tools/v32_readcounts.py <腕名> <走行根（ledgers）> <出力 json> [workers]"""
import collections, concurrent.futures, glob, gzip, json, os, pathlib, sys, time
CODE = pathlib.Path("/Users/tatsu-admin/sfn/sfn-compression-abm-v32main")
MAIN = pathlib.Path("/Users/tatsu-admin/sfn/sfn-compression-abm")
...
ARM, RD, OUT = sys.argv[1], sys.argv[2], pathlib.Path(sys.argv[3])
...
def main():
    ...
    ls = MAIN / f"analysis_pred_2026-09-22/lsweep_{ARM}.json"
    if ls.exists():
