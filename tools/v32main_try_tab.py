...
import os
T0 = Path("/Users/tatsu-admin/sfn/sfn-compression-abm/analysis_v32_2026-09-26/try")
T1 = Path("/Users/tatsu-admin/sfn/sfn-compression-abm/analysis_v32main_2026-09-26/final/try24")
# ★ 版：v3.2-main の試し（2026-09-26 夜）。比べ相手の v3・v3.1b は、午後に同じ種・セルをこのマックで走らせた旗なしの台帳（analysis_v32_2026-09-26/try）
ARMS = [(T0 / "v3_ref", "v3"), (T0 / "v31b_ref", "v3.1b"), (T1 / "v32new_n80_hide", "新しい同化"),
        (T1 / "v32rho0_n80_hide", "ρ だけ 0"), (T1 / "v32cur_n80_hide", "今の同化")]
out = sys.argv[1]
D = {}
secs = {}
for arm, _ in ARMS:
    for side in sorted((arm / "side").glob("*/seed*.jsonl")):
...
L = ["# v3.2-main の試し：誕生・同化・走行末の生存定義の数（このマック、2026-09-26 夜）　判定しない", "",
     "★ 腕：hide（f＝0.5）、NSIM（基準）0.8、seed001・002 × 4 セル（θ′2.1・2.3 × 最頻・sample）。",
     "★ v3 ＝ --nohash --nsim 0.8 --vt 0.3842 --greedy。v3.1b ＝ それに --extend-rule none --charge1 d32（午後に走らせた旗なしの台帳）。",
     "★ 本番の三つ（tools/run_all.sh の段 1）＝ v3.1b ＋ 直し②（--fix2）＋ 名前の順番の直し（--fix-order）＋ --fast ＋ --no-public-history ＋ --dump-slot-history、",
     "  新しい同化 ＝ ＋ --ident-rho 0.5 --ident-argmax --ident-commons、ρ だけ 0 ＝ ＋ --ident-rho 0 --ident-argmax --ident-commons、今の同化 ＝ 旗 A・B・C なし。",
