...
def shuffle_L3(graph: RelationGraph, rng: Random, role_pool: list[str]) -> RelationGraph:
    """役割ユナリーとの対応だけ壊す。共起も引数共有も保つ"""
    out = []
    for r in graph.relations:
        if len(r.arguments) == 1:
            out.append(Relation(r.relation_id, rng.choice(role_pool), r.arguments))
        else:
            out.append(r)
    return RelationGraph(graph.graph_id, graph.entities, tuple(out))
...
def build(n: int, rng_seed: int) -> dict:
    ...
    role_pool = sorted({str(v) for v in seed.data["role_unary"].values()})
    ...
    layers = {
        ...
        "L3_role_correspondence": [shuffle_L3(g, Random(rng_seed + 3000 + i), role_pool)
                                   for i, g in enumerate(real)],
    }
...
def verify(path: Path) -> None:
    ...
    print("""
  読み方
    L2・L3 は述語を引き直さないので、周辺の最大差は 0.0000 になるはず
    L1 は述語を引き直すので、標本ゆらぎのぶんだけ差が出る（0.01 未満が目安）
