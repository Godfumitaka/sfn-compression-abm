"""水位-1・水位-2（事前登録 2026-09-22 20:22:52）と L-4 を、直した模型の既存の種（s1）で判定する。
...
ARMS=[("f=0.00","b2_f00_s1"),("f=0.25","b2_f025_s1"),("f=0.50","b2_hide_s1"),("f=1.00","b2_f10_s1")]
def load(arm):
...
SEEDS=[f"seed{i:03d}" for i in range(1,21)]
print("■ 水位説（★ 直した模型・既存の種 s1）  群S の 外÷内 誤答率（水準3）")
...
def passes(arm):
    ...
    return P
P0=passes("b2_f00_s1"); P5=passes("b2_hide_s1")
