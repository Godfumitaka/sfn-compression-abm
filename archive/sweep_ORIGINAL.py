...
def cell_name(f_value: float, theta: float, scope: str) -> str:
    return f"f{f_value:.4f}_th{theta:.4f}_{scope}"


def parse_cell_name(name: str) -> dict:
    f_part, th_part, scope = name.split("_", 2)
    return {"f": float(f_part[1:]), "theta_prime": float(th_part[2:]), "repair_scope": scope}
