#!/usr/bin/env python3
"""
analyze.py — 本走行の一次解析（rev6・2026-08-30）

rev5 からの差分は 3 箇所。★ 既存の数値を出す経路には触れていない
  1  predicate_split() を追加（走行末の生存述語と墓石述語を分ける補助関数）
  2  per_def に alive_preds / dead_preds を追加
     → ★「生き残るのは汎用述語か」を 178,000 本の規模で言える
     → 台帳の読み：塔 allow は四モチーフ全部にある（π_tower = 1.0）。
       cause は M2・M3・M4。8/28 実測の神話は m=2 で allow ＋ cause だった
  3  冒頭の版表示

★ 検収の不変条件：analyze_rev5.log と analyze_rev6.log の差分は
  出力パス表示の 1 箇所だけであること。画面に出る数値は 1 つも動かない

rev4 からの差分は 5 箇所。★ 既存の数値を出す経路には触れていない
  1  change_times（構成素が動いた試行の列）を per_def に追加
     → 検算1 を「報告地平の窓の内側」で数え直せる。rev4 では走行末まで見ていた
  2  born（def が最初に現れた試行）を追加 → 薄化を年齢で層別できる
  3  m_alloc（墓石を含む構成素の総数）を追加 → 薄化を m_alloc − m_live で測れる
  4  ★ OA_gstar_frac を追加：欠けた述語の「割合」。二値版は m_live とともに
     1 へ飽和する（和集合の効果）。二値版は撤回せず併記する
  5  n_alive_predicates を追加（m_live との突き合わせ用）

★ 検収の不変条件：analyze_rev4.log と analyze_rev5.log の差分は
  (i) OA の表に OA_gstar_frac の 1 行が増える (ii) 出力パス表示
  の 2 箇所だけであること。既存の数値が 1 桁でも動けば汚染している

rev3 からの差分は次の三点だけ。★ 数値を出す経路には一切触れていない
  1  per_def に last_change を追加（U-089 の吸収検査に要る。rev3 は e3_estimators が
     返しているのに記録していなかった）
  2  OA の説明文が旧定義「一本も無かった」のままだったのを実装に合わせた
  3  「どれを採るかは未決」を 2026-08-30 の宣言（主 = gstar）に置き換えた

★ 検収の不変条件：analysis/ と analysis2/ で、last_change 以外のすべての欄と
  画面に出るすべての数値が完全に一致すること。一致しなければ rev4 の編集が汚染している
...
def e3_estimators(series, window, horizon):
    ...
    return {
        ...
        "change_times": changes,          # rev5 追加（窓の内側で吸収を数え直すため）
        "born": born,                     # rev5 追加（薄化を年齢で層別するため）
        "window": window,
    }
...
def predicate_split(states, registration):
    """走行末の (生存述語, 墓石述語) を返す。★ T-045 の三つ組キーで突き合わせる。

    registration の構成素は述語を持つが alive は登録時点の値なので、
    生死は走行末の constituent_states 側で決める（is_structurally_thin と同じ作り）。
    """
    if not registration:
        return None, None
    table = {
        (c.get("slot_index"), c.get("registered_at")): c.get("predicate")
        for c in (registration.get("constituents") or ())
    }
    alive, dead = [], []
    seen = set()
    for constituent in states:
        key = (constituent.get("slot_index"), constituent.get("registered_at"))
        seen.add(key)
        name = table.get(key)
        if name:
            (alive if constituent.get("alive") else dead).append(name)
    # constituent_states に現れない登録済み構成素は墓石として扱う
    for key, name in table.items():
        if key not in seen and name:
            dead.append(name)
    return sorted(alive), sorted(dead)
...
def over_application(applied, supply, motifs, predicates, alive_predicates):
    ...
    if predicates is not None and alive_predicates:
        ...
        # rev5 追加：欠けた述語の「割合」。★ 二値版は m_live とともに 1 へ飽和する
        #   （和集合の効果。E[二値] = 1 − (1−p)^m ／ E[割合] = p で m に依らない）
        shares = [len(alive_predicates - predicates[t]) / len(alive_predicates)
                  for t in applied if t < len(predicates)]
        if shares:
            out["OA_gstar_frac"] = sum(shares) / len(shares)
        out["n_alive_predicates"] = len(alive_predicates)
    return out
...
def analyse_run(path, use_world=True):
    ...
    for name in series:
        ...
        record = {
            ...
            "alive_preds": predicate_split(last_states.get(name, []),
                                           registrations.get(name))[0],
            "dead_preds": predicate_split(last_states.get(name, []),
                                          registrations.get(name))[1],
            "m_live_final": series[name][-1][1] if series[name] else 0,
            "n_changes": estimators.get("n_changes"),
            "last_change": estimators.get("last_change"),   # rev4 追加（U-089 の吸収検査）
            "change_times": estimators.get("change_times"),   # rev5 追加
            "born": estimators.get("born"),                   # rev5 追加
            "m_alloc": len((registrations.get(name) or {}).get("constituents") or ()),
...
def main():
  ...
  supply  その def を作った場面の最頻モチーフを担当とする
  applied その def が使われた場面の最頻モチーフを担当とする
  gstar   ★ 対応づけない。適用場面の G* に生存構成素の述語が ★ 一本でも欠けていた 割合
          （「一本も無い」だと適用の条件から常に 0 になるため。2026-08-29 に締め直した）
  ★ 主は gstar（2026-08-30 宣言）。supply は副。applied は 1 −（最頻モチーフ占有率）に
    等しく取りうる最小値なので、過剰適用の証拠には使わない
""")
    for tag in ("OA_supply", "OA_applied", "OA_gstar", "OA_gstar_frac"):
